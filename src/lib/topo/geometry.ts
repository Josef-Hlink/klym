...
// camera) and returns numeric paint-ready primitives (`pts`/`verts`/
// `quads`, viewBox units) for the canvas painter via scene.ts.
//
...
export type BlockFace = { verts: P2[]; depth: number };
...
export type PolylineRun = { pts: P2[]; color: string; depth: number };
...
export type DrapePath = { topPts: P2[]; quads: [P2, P2, P2, P2][] };

// Build the "drape" geometry for a single bin: per-segment quads from the
// route surface down to its shadow at refFrame.minEle, plus the top edge
// as a polyline (used for the hover halo).
export function buildDrape(
	...
	if (!refFrame) return { topPts: [], quads: [] };
	const groundAt = groundEleAt ?? flatGround(refFrame);
	...
	for (let i = 0; i < tops.length - 1; i++) {
		...
		quads.push(cross >= 0 ? [A, B, C, D] : [A, D, C, B]);
	}

	return { topPts: tops, quads };
}

export type HoverHighlight = DrapePath & { color: string };
...
export function buildHoverHighlight(
	...
	const empty: HoverHighlight = { topPts: [], quads: [], color: '' };
	...
	if (!bin) return empty;
	const drape = buildDrape(bin, points, slicedPoints, refFrame, project, groundEleAt);
	return { ...drape, color: gradeColor(bin.grade, theme) };
}

export type DrapeColored = { quads: [P2, P2, P2, P2][]; color: string };
...
export function buildAllDrapes(
	...
	return bins.map((bin) => ({
		quads: buildDrape(bin, points, slicedPoints, refFrame, project, groundEleAt).quads,
